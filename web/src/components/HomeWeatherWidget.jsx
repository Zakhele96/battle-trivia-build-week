...
export default function HomeWeatherWidget() {
  ...
  async function loadWeather() {
    try {
      ...
      const forecastResponse = await fetch(
        `https://api.open-meteo.com/v1/forecast?latitude=${place.latitude}&longitude=${place.longitude}&timezone=Africa%2FJohannesburg&current=temperature_2m,weather_code&daily=temperature_2m_max,temperature_2m_min`
      );
  ...
  return (
    <section className="mt-6 border border-slate-200 bg-white p-4 lg:hidden">
      <div className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">
        Weather
      </div>

      {loading ? (
        <div className="mt-3 text-sm text-slate-600">Loading weather...</div>
      ) : error ? (
        <div className="mt-3 text-sm text-red-700">{error}</div>
      ) : weather ? (
        <>
          <div className="mt-1 text-xs text-slate-500">Today</div>
          <div className="mt-1 text-sm text-slate-600">{formatToday()}</div>
          ...
          <div className="mt-4 flex items-center justify-between gap-4">
            ...
            <div className="text-right text-xs text-slate-600">
              <div>{weather.placeName}</div>
              <div className="mt-1">
                H {Math.round(weather.daily.max)}° / L {Math.round(weather.daily.min)}°
              </div>
            </div>
          </div>
        </>
