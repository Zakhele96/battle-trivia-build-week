...
export default function AdminEditPlacementPage() {
  const { id } = useParams();
  ...
  async function loadPlacement() {
    try {
      ...
      const data = await getAdminPlacementById(id);

      setPlacement(data);
      setForm({
        title: data?.title || "",
        placementKey: data?.placementKey || "homepage_hero_promo",
  ...
  return (
    <>
      <AdminPageShell
        ...
        description="Update promo, ad slot, and partner placement details."
        actions={
          <Link
            to="/admin/placements"
      ...
      >
        <div className="space-y-4">
          ...
          <AdminCard>
            {loading ? (
              <div className="text-sm text-neutral-600">Loading placement...</div>
            ) : (
